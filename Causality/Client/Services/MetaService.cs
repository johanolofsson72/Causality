...
namespace Causality.Client.Services
{
    ...
    {
        Causality.Shared.Models.MetaService.MetaServiceClient _metaService;
        ...
        /// <summary>
        /// TryGet, Includes (none), OrderBy (Id, Key, Value, UpdatedDate)
        /// </summary>
        ...
        /// <returns></returns>
        public async Task TryGet(Expression<Func<Meta, bool>> filter, string orderby, bool ascending, Func<IEnumerable<Meta>, string, Task> onSuccess, Func<Exception, string, Task> onFail, CascadingAppStateProvider state)
        {
            try
            {
                ...
                string key = ("causality_meta_tryget_" + filterString + "_" + orderby + "_" + ascending.ToString()).Replace(" ", "").ToLower();
                List<Meta> data = new();
                ...
                if (state.AppState.UseIndexedDB)
                {
                    ...
                    {
                        data = JsonConvert.DeserializeObject<List<Meta>>(result.Value);
                        source = "indexedDB";
                    }
        ...
        /// <param name="state"></param>
        /// <returns></returns>
        public async Task TryGetById(int id, Func<Meta, string, Task> onSuccess, Func<Exception, string, Task> onFail, CascadingAppStateProvider state)
        {
            try
            {
        ...
        public async Task TryInsert(Meta meta, Func<Meta, string, Task> onSuccess, Func<Exception, string, Task> onFail, CascadingAppStateProvider state)
        {
            try
            {
                string status = "";
                ...
                if(onSuccess is not null) await onSuccess(meta, status);
        ...
        public async Task TryUpdate(Meta meta, Func<Meta, string, Task> onSuccess, Func<Exception, string, Task> onFail, CascadingAppStateProvider state)
        {
            try
            {
                string status = "";
