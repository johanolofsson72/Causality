...
namespace Causality.Client.Services
{
    public class UserService
    {
            ...
            try
            {
                if (state.AppState.UseIndexedDB)
                {
                    if (await _onlineState.IsOnline())
                    {
                        ...
                        if(onSuccess is not null) await onSuccess(ret.Status);
                        return;
                    }
                    else
                    {
        ...
        /// <returns></returns>
        public async Task TryGet(Expression<Func<User, bool>> filter, string orderby, bool ascending, string includeProperties, Func<IEnumerable<User>, string, Task> onSuccess, Func<Exception, string, Task> onFail, CascadingAppStateProvider state)
        {
            try
            {
        ...
        /// <param name="state"></param>
        /// <returns></returns>
        public async Task TryGetById(int id, string includeProperties, Func<User, string, Task> onSuccess, Func<Exception, string, Task> onFail, CascadingAppStateProvider state)
        {
            try
            {
        ...
        public async Task TryInsert(User User, Func<User, string, Task> onSuccess, Func<Exception, string, Task> onFail, CascadingAppStateProvider state)
        {
            try
            {
                string status = "";
                ...
                if(onSuccess is not null) await onSuccess(User, status);
        ...
        public async Task TryUpdate(User User, Func<User, string, Task> onSuccess, Func<Exception, string, Task> onFail, CascadingAppStateProvider state)
        {
            try
            {
                string status = "";
